...
Check to see what size fish transitions from O to YP

```{r fish size}
add_colors <- fish_anem_dive() %>% 
  filter(fish_table_id %in% fish_locs$fish_table_id) %>% 
  select(fish_table_id, size, color, date)

fish_colors <- left_join(fish_locs, add_colors, by = "fish_table_id") %>% 
  filter(!is.na(color))
...
# these need to be checked in case some are same season color changes

# working on this list manually is going to take forever, must automate
# recap_id_changed_from_O_to_YP <- c(30, 32, 40, 47, 52, 61, 63, 70, 71, 72, 75, 77, 81, 98, 101, 106, 115, 117, 118)
recap_id_changed_from_O_to_YP <- c()
for (i in seq_along(diff_colors$recap_id)){
  x <- fish_diff %>% 
    filter(recap_id == diff_colors$recap_id[i]) %>% 
    select(recap_id, size, color, date) %>% 
    arrange(date) 
 ...
 if("O" %in% col$color){
   if(x$color[nrow(x)] != "O"){
     recap_id_changed_from_O_to_YP <- c(recap_id_changed_from_O_to_YP, diff_colors$recap_id[i])
   }
 }
}

male_to_female <- fish_diff %>% 
  filter(
    # only fish that changed from O to YP
    recap_id %in% recap_id_changed_from_O_to_YP, 
    # only the YP sizes
    color == "YP"
    )
# narrow down to the first date the fish was YP if there was more than one date
for (i in male_to_female$recap_id){
  x <- male_to_female %>% 
    filter(recap_id == i) %>% 
    arrange(date)
  ...
  if(nrow(x) > 1){
    ...
    male_to_female <- male_to_female %>% 
      filter(!fish_table_id %in% y$fish_table_id)
  }
}
```
Plot the distribution of sizes
```{r plot size}
c <- ggplot(male_to_female, aes(as.numeric(size)))

c + geom_area(stat = "bin")+ 
  theme_classic()+
  labs(title = "Distribution of sizes at color change from O to YP", 
       subtitle = "These data have not been photo verified and some may represent fish that \n were marked as both O and YP in the same field season.  Use with caution.", 
       x = "Size at YP", 
       y = "count") 

ggsave(filename = here("plots", "distribution-size-color-change.pdf"))
```
