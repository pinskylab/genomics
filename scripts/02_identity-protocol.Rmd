...
# Examine new matches
```{r}
new_pairs <- anti_join(idcsv, old_pairs, by = c("first_sample_id", "first_tag_id", "first_gen_id", "second_sample_id", "second_tag_id", "second_gen_id", "matching_loci", "mismatching_loci", "first_lat", "first_lon", "second_lat", "second_lon", "first_sex", "second_sex")) %>% 
...
saveRDS(new_gen_ids, "~/Documents/leyteBuildDB/data/new-gen-ids_2019-04-16.RData")
```
...
# # which gen_ids are duplicated and which have the most loci?
# dups <- gen_ids %>% 
#   group_by(gen_id) %>% 
#   count() %>% 
#   filter(n > 1) %>% 
#   left_join(numgen, by = "gen_id") %>% 
#   group_by(gen_id) %>% 
#   mutate(leastloci = min(numloci))
# 
# # remove the rows that don't have the most loci
# least_loci <- dups %>% 
#   filter(numloci == maxloci)
# 
# # which are still duplicated?
# still_dup <- most_loci %>% 
#   group_by(gen_id) %>% 
#   count() %>% 
#   filter(n>1) 
# 
# mostest_loci <- most_loci %>%
#   filter(gen_id %in% still_dup)
#   mutate(max_lig = max(ligation_id))


```
2019-04-16 - The first time I tried to run this today, it assigned more than one gen_id to the same sample id. Fixing below:
```{r eval=FALSE}
dup_samp <- gen_ids %>% 
  group_by(sample_id) %>% 
  count() %>% 
...
dup_gens <- gen_ids %>% 
  filter(sample_id %in% dup_samp$sample_id)

# double check which gen_ids are recaptures
gen_db <- get_fish() %>% 
  filter(gen_id %in% dup_gens$gen_id, 
         !is.na(gen_id))

```
Duplicate gen_ids were fixed in the database (leyteBuildDB) 2019-04-16
APCL17_375 has 4 different anem_table_ids and fish_table_ids - fixing this over in the database code.
```{r eval=FALSE}
too_many_rows <- fish_anem_dive() %>% 
  filter(sample_id == "APCL17_375")
```



** Not sure how to remove fish from the genepop without losing valuable data, have started a github issue #19 to ask Katrina what she wants me to do**
