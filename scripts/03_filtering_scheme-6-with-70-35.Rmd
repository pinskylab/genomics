...
```{r message=FALSE}
read.table(here("data", "step3AB.AB"),
           col.names = "AB", stringsAsFactors = FALSE) %>%
  ggplot(aes(x = AB)) +
  geom_histogram(binwidth = 0.01, color = "black", fill = "grey95") +
  geom_vline(xintercept = 0.5, color = "red", linetype = "dashed") +
...
# depth
depth <- read.table(here("data", "step3.DEPTH"),
                    col.names = "depth")

# quality score
qual <- read.table(here("data", "step3.QUAL"),
...
# depth
depth <- read.table(here("data", "step3ABQDPTH.DEPTH"),
                    col.names = "depth")

# quality score
qual <- read.table(here("data", "step3ABQDPTH.QUAL"),
                   col.names = c("locus", "pos", "qual"))
...
temp <- read.table(here("data", "step3.MQM"), col.names = "MQM")

mapqual <- read.table(here("data", "step3.MQMR"), col.names = "MQMR")
...
# ```{r eval=FALSE}
# 
# temp <- read.table(here("data", "step3ABQDPTH.MQM"), col.names = "MQM")
# 
# mapqual <- read.table(here("data", "step3ABQDPTH.MQMR"), col.names = "MQMR")
# 
# mapqual <- bind_cols(mapqual, temp) %>%
#   mutate(ratio = MQM/MQMR)
# 
...
depth <- read.table(here("data", "step3ABQDPTHQUAL.ldepth.mean"),header = TRUE, stringsAsFactors = FALSE)
...
imiss <- read.table(here("data", "step4a.imiss"), 
...
imiss <- read.table(here("data", "step5a.imiss"), 
                    header = TRUE, stringsAsFactors = FALSE)
