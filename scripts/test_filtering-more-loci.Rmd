...
This chunk can take some time because it is filtering out the full data set.
...
Retain loci with genotype call rate > 50% and flag individuals with missing data.
```{bash, message=FALSE}
cd ../
...
```{r plot AB}
read_delim(here("results", "minQ20minDP5meanDP15geno50ind90.AB"), delim = "\t") %>%
  ggplot(aes(x = AB)) +
