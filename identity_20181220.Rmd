...
<!-- genfile <- paste0("data/", params$data_dir, "/seq33_03_baits_only_SNPs.gen") -->
...
<!-- # count the number of loci per individual - takes 7 minutes -->
<!-- Sys.time() -->
<!-- test <- genedf %>%  -->
<!--   rowwise() %>%  -->
<!--   group_by(names) %>%  -->
<!--   summarise(numloci = sum(!is.na(.))) -->
<!-- Sys.time() -->
...
<!-- genedf <- genedf %>%  -->
<!--   separate(names, c("spp", "id"), sep = "_") %>%  -->
<!--   select(-spp) %>%  -->
<!--   separate(id, c("id", "F"), sep = "\\.") %>%  -->
<!--   select(-F) -->

<!-- lose <- left_join(select(idcsv, first_id, second_id), id_matches, by = c("first_id"= "id")) -->

<!-- lose <- lose %>%  -->
<!--   rename(first_num_loci = numloci) -->

<!-- lose <- left_join(lose, id_matches, by = c("second_id"= "id")) %>%  -->
<!--   rename(sec_num_loc = numloci) -->

<!-- lose <- lose %>%  -->
<!--   mutate(drop = ifelse(first_num_loci >= sec_num_loc, second_id, first_id)) -->
